...
int main()
{
	...
	dstBuf=nmppsMalloc_8s(maxSize+256);
	src=nmppsAddr_8s(srcBuf,128);
	dst=nmppsAddr_8s(dstBuf,128);
	
	nmppsRandUniform_8s(srcBuf,maxSize+256);
	nmppsRandUniform_32s(pWeights,MAX_KERNEL_SIZE+1);
	nmppsSet_8s(0xCCCC,dstBuf,maxSize+256);
	
	
	for(int size=0; size<maxSize; size+=8){
		for(int kernelSize=3; kernelSize<33; kernelSize+=2){
			if (nmppsFIRInitAlloc_8s(&state,pWeights,kernelSize))
				return -1;
			nmppsFIR_8s(src,dst,size,state);
			nmppsCrcAcc_8s(dst,size,&crc);
			nmppsFIRFree(state);
		}
	}
