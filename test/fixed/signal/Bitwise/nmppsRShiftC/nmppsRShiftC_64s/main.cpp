...
int main()
{
	...
	int	MaxLongSize = 128;
	MaxLongSize=MIN(MaxLongSize,LONG2long(SizeL0));
	MaxLongSize=MIN(MaxLongSize,LONG2long(SizeG0));
	...
	for(int LongSize=0;LongSize<MaxLongSize;LongSize+=NMPP_MIN_REP)
	{
		...
		nmppsRShiftC_64s((nm64s*)L0,Shift,(nm64s*)G0,LongSize);
		nmppsCrcAcc_32u((nm32u*)G0,LongSize*2,&crc);
	}
	
	//! \fn void nmppsRShiftC_64s(nm64s*,int, nm64s*,int)
