...
/* The using example of forward FFT4096 for floating point */ 
...
int main()
{
  ...
  // a special fft var which has to will is initialized by nmppsFFT4096InitAlloc_32fcr
  // специальная переменная БПФ, которая должна быть инициализирована с помощью nmppsFFT4096InitAlloc_32fcr
  NmppsFFTSpec_32fcr* spec;
  
  // an initializing of a special fft var
  // инициализация специальной переменной БПФ
  int ret_val = nmppsFFT4096InvInitAlloc_32fcr(&spec);
  if(ret_val) {
	printf("Error %d!\n", ret_val);
	return 1;
  }
  
  // computing forward fft4096 from a generated sin func
  // вычисление прямого БПФ4096 от сгенерированной функции синуса
  clock_t t0=clock();
  nmppsFFT4096Inv_32fcr(src, dst, spec);
  clock_t t1=clock();
