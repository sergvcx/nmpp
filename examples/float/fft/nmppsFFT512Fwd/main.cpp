...
/* The using example of forward FFT512 for floating point */ 
...
int main()
{
  ...
  // a special fft var which has to will is initialized by nmppsFFT512InitAlloc_32fcr
  // специальная переменная БПФ, которая должна быть инициализирована с помощью nmppsFFT512InitAlloc_32fcr
  NmppsFFTSpec_32fcr* spec;
  
  // an initializing of a special fft var
  // инициализация специальной переменной БПФ
  int ret_val = nmppsFFT512FwdInitAlloc_32fcr(&spec);
  if(ret_val) {
	printf("Error %d!\n", ret_val);
	return 1;
  }
  ...
  nmppsFFT512Fwd_32fcr(src, dst, spec);
