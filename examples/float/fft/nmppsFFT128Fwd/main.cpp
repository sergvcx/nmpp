...
int main()
{
  ...
  // a special fft var which has to will is initialized by nmppsFFT128FwdInitAlloc_32fcr
  // специальная переменная БПФ, которая должна быть инициализирована с помощью nmppsFFT128FwdInitAlloc_32fcr
  NmppsFFTSpec_32fcr* spec;
  
  // an initializing of a special fft var
  // инициализация специальной переменной БПФ
  int ret_val = nmppsFFT128FwdInitAlloc_32fcr(&spec);
  if(ret_val) {
	printf("Error %d!\n", ret_val);
	return 1;
  }
  ...
  clock_t t0=clock();
  nmppsFFT128Fwd_32fcr(src, 1, dst, 1, spec);
  clock_t t1=clock();
