...
int main()
{
  ...
  // a special fft var which has to will is initialized by nmppsFFT1024InitAlloc_32fcr
  ...
  int ret_val = nmppsFFT1024InvInitAlloc_32fcr(&spec);
  if(ret_val) {
	printf("Error %d!\n", ret_val);
	return 1;
  }
  ...
  clock_t t0=clock();
  nmppsFFT1024Inv_32fcr(src, dst, spec);
  clock_t t1=clock();
