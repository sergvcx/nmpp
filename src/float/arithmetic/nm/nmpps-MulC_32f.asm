...
//*                                                                         */
//*   File:             nmpps-MulC_32f.asm                          	 	*/
//*   Contents:         The function computes y = Const * x   				*/
//***************************************************************************/

global _nmppsMulC_32f: label;
...
<_nmppsMulC_32f>
	...
	if =0 delayed goto less64_MulC_32f;
...
<MulC_32f>
	...
	if =0 delayed goto exit_MulC_32f;					// если остаток 0, значит обработаны все элементы
<less64_MulC_32f>
		nul;
		gr7 >>= 1;										// проверяем, 1 в остатке или больше
	if =0 delayed goto MulC_32f_OneElement;
		gr7--;
		vlen = gr7;
...
<MulC_32f_OneElement>
	gr1 <<= 31;
	if =0 delayed goto exit_MulC_32f;
		ar2, gr2 = [ar1];
