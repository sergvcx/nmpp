...
<_nmppsConvert_32u32fcr>
	...
	gr0 - 1;
	if =0 delayed goto Packing_32s32fcr_1elem;
		gr1 = 4;
	
	ar2 = ar1 + 2;
	
	gr3 = gr0 >> 6;	// count of the iterations
	if =0 delayed goto Packing_32s32fcr_less64;
		gr2 = gr1;
		gr4 = gr0;

<Packing_32s32fcr>					// N > 64
	fpu 0 rep 32 vreg0 = [ar0++];
	fpu 0 .packer = vreg0 with .float .in_high <= .fixed_32 .in_low;
	fpu rep 32 [ar1++gr1] = .packer;
	gr3--;
	if > delayed goto Packing_32s32fcr;
		fpu 0 .packer = vreg0 with .float .in_high <= .fixed_32 .in_high;
		fpu rep 32 [ar2++gr2] = .packer;
	gr4 = gr4 << 26;				// computing a reminder
	gr4 = gr4 >> 26;				// computing a reminder
	if =0 goto exit_Conv32s32fcr;

<Packing_32s32fcr_less64>			// N < 64
	gr4 = gr4 >> 1;
	gr4--;
	vlen = gr4;
	...
	gr0 = gr0 << 31;
	if =0 delayed goto exit_Conv32s32fcr; // if N is even then go to exit
		fpu 0 .packer = vreg0 with .float .in_high <= .fixed_32 .in_high;
		fpu rep vlen [ar2++gr2] = .packer;

<Packing_32s32fcr_1elem>				  // if N is odd 
