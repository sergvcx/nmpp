...
static int GetMatrixCount(int nSize)
{
	return (nSize+5)/4;	
}
...
int nmppiFilterInit_16s32s(int* pWeights, int nKerWidth, int nKerHeight, int nImgWidth, NmppiFilterState* pKernel)
{
	...
	nmppsSet_32s(0,(nm32s*)pWeightMatrix,(nNumberOfArrays0+nNumberOfArrays1)*nKerHeight*4*2);
...
int nmppiFilterGetStateSize_16s32s(int nKerWidth, int nKerHeight)
{
	...
	AllocSize=6+nDispArraySize+(nNumberOfArrays0+nNumberOfArrays1)*nKerHeight*4*2;
	return AllocSize;
}
