...
//
//  $Revision: 1.4 $      $Date: 2005/01/22 13:50:27 $
//
//! \if file_doc
//!
...
// AR2=gr5
    ...
    //------------------------------------------------------------------------
    //! \fn void core_ClipMulNDNW2_AddVr(nmreg nb1, nmreg sb, nmreg f1cr, nmreg vr, nmreg ar0, nmreg gr0, nmreg ar1, nmreg gr1, nmreg ar4, nmreg gr4, nmreg gr5, nmreg ar6, nmreg gr6);
    //!			 
    //------------------------------------------------------------------------

global core_ClipMulNDNW2_AddVr:label;
<core_ClipMulNDNW2_AddVr>
.branch;

	push ar5,gr5;
	ar3 = ar4 with					// Store pointer to weight array  
	gr7 = gr5>>5;					// Count of blocks by rep 32
	rep 2 wfifo=[ar4++],ftw,wtw;			// preload matrix for First_vsum

	ar5 = ar1 with					// Store pointer to array of displacemts
	gr3 = gr1 noflags;				// Store number of arrays
	gr5 = ar0;						// Anchor pointer
//	ar1 = AR1;						// Restore pointer to array of displacemts
//	gr1 = gr3;						// Restore number of arrays
	gr2 = [ar1++]	with gr1-- noflags;			// Read first disp   , Dec number of arrays
	ar2 = gr5	with gr7;				// Restore anchor pointer
	ar0 = ar2 + gr2	with gr1-- noflags;			// Set  first pointer, Dec number of arrays
	
	
	if =0 delayed goto Rep_N;
		rep 2 wfifo=[ar4++],ftw;		// Preload matrix for Next_vsum
		nul;
	<Next_Packet>
		// <First_Block>
		...
		wtw;
		rep 32 data=[ar0++gr0] with vsum ,activate data,afifo;
		rep 2 wfifo=[ar4++],ftw;		// Preload matrix for First_Block
		//<First_Block>
	...
	if > delayed goto Next_Packet;
		rep 2 wfifo=[ar4++],ftw;		// Preload matrix for Next_Block
