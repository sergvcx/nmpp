...
macro VEC_SUB_ABS1_REP(N)
		rep N data=[ar0++gr0]	with data;
		rep N data=[ar1++gr1]	with afifo-data;
	delayed return;
		rep N [ar4],ram =afifo	with activate afifo xor afifo;
		//rep N					with vsum afifo,ram,ram;			
		//rep N [ar6++gr6]= afifo;
	nul;nul;
end   VEC_SUB_ABS1_REP;

SubAbs1_rep0:label;

    //------------------------------------------------------------------------
    //! \fn void core_SubAbs(nmreg nb1, nmreg sb, nmreg f1cr, nmreg ar0, nmreg gr0, nmreg ar1, nmreg gr1, nmreg ar4, nmreg gr5, nmreg ar6, nmreg gr6)
    //!
    //------------------------------------------------------------------------

global core_SubAbs1 :label;
<core_SubAbs1>
.branch;
	push ar2,gr2 with gr2=gr5<<27 ;
	push ar5,gr5 with gr5>>=5;	
	if =0 delayed goto SubAbs_repN with gr2>>=24;
		ar2 = SubAbs1_rep0 with gr5--;
	<next_SubAbs1_rep32>
	...
	if <>0 delayed  goto next_SubAbs1_rep32 with gr5--;
		nul;
