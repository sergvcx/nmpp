...
.if (NMPP_MIN_REP <= N);
	delayed return;
		rep N ram = [ar0++gr0] ;
		rep N data = [ar1++gr1] with data or ram;
		rep N [ar6++gr6] = afifo;
.endif;
end   VEC_OR_REP;
...
Or_rep0:label;
    ...
    //------------------------------------------------------------------------
    //! \fn void core_Or(nmreg ar0, nmreg gr0, nmreg ar1, nmreg gr1, nmreg gr5, nmreg ar6, nmreg gr6)
    //!
    //------------------------------------------------------------------------

global core_Or :label;
<core_Or>
.branch;
	...
	<next_Or_rep32>
		rep 32 ram = [ar0++gr0] ;
		rep 32 data = [ar1++gr1] with data or ram;
