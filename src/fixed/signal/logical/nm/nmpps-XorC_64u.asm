//------------------------------------------------------------------------
//
//  $Workfile:: Or.as $
//
//  Neuro Matrix Performance Primitives
//
//  Copyright (c) RC Module
//
//  $Revision: 1.1 $      $Date: 2004/11/22 13:50:05 $
//
//! \if file_doc
//!
//! \file   Or.asm
//! \author S.Mushkaev
//! \brief  Логические функции над векторами.
//!
//! \endif
//!
//------------------------------------------------------------------------
   

extern core_data_xor_ram:label;
begin ".text_nmplv"

/////////////////////////////////////////////////////////////////////////////


//! \fn void nmppsXorC_64u(nm64u* pSrcVec, uint64b nVal, nm64u* pDstVec, int nSize);
//!
//! \perfinclude _nmppsXorC_64u.html

global _nmppsXorC_64u:label;
<_nmppsXorC_64u>
	ar5 = ar7 - 2	with gr7=false;
	push ar0,gr0	with gr7++;
	push ar1,gr1	with gr7++;
	push ar5,gr5	with gr0=gr7;
	push ar6,gr6	with gr6=gr7;

	ar0 = [--ar5];		//	nm64*		pSrcVec1,	// Input Buffer					:long Local[nSize]
	//ar1 = [--ar5];		//	nm64*		SrcN,		// Input Buffer					:long Local[1]
	ar1 = ar5-3;
	ar5 = ar1;
	delayed call core_data_xor_ram;
		ar6 = [--ar5];	//	nm64*		pDstVec,		// Output Buffer				:long Global[nSize
		gr5 = [--ar5];	//	int			nSize		// VectXXor size in 64-bit wXXords	:nSize=[0,1,2,..,n

	pop ar6,gr6;
	pop ar5,gr5;
	pop ar1,gr1;
	pop ar0,gr0;
return;
.wait;
end ".text_nmplv";
