...
//t1=0;
	...
	// в array S[i],where i=0..2047 (S[i]=[S[i].imag|S[i].real]), 
...
.endrepeat;
	...
	// в array T[i],where i=0..1023 (T[i]=[T[i].imag|T[i].real]), 
...
.endrepeat;
	...
	// at the I-st stageе T is transformed into array YY[i],where i=0..2047 (YY[i]=[YY[i].imag|YY[i].real]), 
	// and at the II-nd stageе array YY is transformed into array Y
