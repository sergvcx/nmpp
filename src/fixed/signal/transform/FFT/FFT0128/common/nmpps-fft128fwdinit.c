...
//#include "rpc-host.h"	
	...
	void nmppsFFT128Free(NmppsFFTSpec* spec )
	{
	#ifdef RPC
...
*/
	...
	nm32sc fixW128(int arg, int max){
		nm32sc res;
		double argIm= -2.0*PI/128.0*arg;
	...
	int nmppsFFT128FwdInitAlloc(  NmppsFFTSpec* spec, int settings)
	{
		int i=0;
		spec->buffer[0]  =nmppsMalloc_32sc(128);
		spec->buffer[1]  =nmppsMalloc_32sc(128);
		spec->fftTable[0]=nmppsMalloc_16s(8*4*2);	//128 Bytes
		spec->fftTable[1]=nmppsMalloc_8s(128*8*2);	//1024 Bytes
		spec->shift [0]=-1;
		spec->free=free;
...
/*
	...
	*/
/*
	int nmppsFFT32FwdOptimize(void* src, void* dst, fseq128* allocOrder) 
	{
		unsigned heapIndx0;
		unsigned heapIndx1;
		fseq128   route;
