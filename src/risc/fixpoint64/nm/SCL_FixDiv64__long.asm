...
// Возвращаемое значение функцией IdivInv64 соответствует  частному 
// от целочисленного деления со знаком  (Divisor<<(DivItearations-1)) на Divisor.
//----- Параметры --------
// long* Dividend -64 разрядное целое делимое со знаком
// long* Divisor -64 разрядный целый делитель со знаком .
...
// может находиться в пределах [1,63] и дожно быть нечетным числом.
// ---- Примечание ---------
// Чтобы старший значащий бит частного не выходил за пределы разрядной 
// сетки т.е. за пределы 62 бита, необходимо чтобы значение выражения 
...
gr1++;
	...
	//------- коррекция значения и знака частного -----------
	if >= delayed goto SkipResultNegation;
		rep 1 with afifo-1;				// коррекция младшего бита частного 
