...
//! \fn void nmppsMulC_AddV_AddC(nm32s*,int,nm32s*,int,nm32s*,int)
...
int main()
{
	...
	MaxIntSize=MIN(MaxIntSize,SizeL1*8);
	...
	for(int IntSize=0;IntSize<=MaxIntSize;IntSize+=2*NMPP_MIN_REP)
	{
		int MulN=nmppcRand();
		int AddN=nmppcRand();
		nmppsMulC_AddV_AddC_32s((nm32s*)L0,MulN,(nm32s*)L1,AddN,(nm32s*)G0,IntSize);	
