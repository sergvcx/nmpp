#include "nmpp.h"
#include "minrep.h"


	

///////////////////////////////////////////////////////////////////////////////////////
nm64s *L0;
nm64s *L1;
nm64s *L2;
nm64s *L3;

nm64s *G0;
nm64s *G1;
const int KB=1024/8;
const int SizeL0=30*KB;
const int SizeL1=30*KB;
const int SizeL2=30*KB;
const int SizeL3=30*KB;
const int SizeG0=30*KB;


int main()
{
	

	L0=nmppsMalloc_64s(SizeL0);
	L1=nmppsMalloc_64s(SizeL1);
	L2=nmppsMalloc_64s(SizeL2);
	L3=nmppsMalloc_64s(SizeL3);
	G0=nmppsMalloc_64s(SizeG0);

	if ((L0==0)||(L1==0)||(L2==0)||(L3==0)||(G0==0))
		return -1;

	unsigned int crc = 0;

	int	MaxLongSize=1024;
	MaxLongSize=MIN(MaxLongSize,SizeL0);
	MaxLongSize=MIN(MaxLongSize,SizeL1);
	MaxLongSize=MIN(MaxLongSize,SizeG0);

	nmppsRandUniform_32u((nm32u*)L0,SizeL0*2);
	nmppsRandUniform_32u((nm32u*)L1,SizeL1*2);
	nmppsRandUniform_32u((nm32u*)L2,SizeL2*2);
	nmppsRandUniform_32u((nm32u*)L3,SizeL3*2);
	nmppsSet_32s((int)0xCCCCCCCC,(nm32s*)G0,(SizeG0*2));
	
	for(int LongSize=0;LongSize<=MaxLongSize;LongSize+=NMPP_MIN_REP)
	{
		nmppsOr4V_64u((nm64u*)L0,(nm64u*)L1,(nm64u*)L2,(nm64u*)L3,(nm64u*)G0,LongSize);	
		nmppsCrcAcc_32u((nm32u*)G0,MIN(LongSize+128,SizeG0),&crc);
	}
	//! \fn void nmppsOr_64u(nm64u*,nm64u*,nm64u*,int)		

	nmppsFree(L0);
	nmppsFree(L1);
	nmppsFree(L2);
	nmppsFree(L3);
	nmppsFree(G0);


	

	return (crc>>2)^0;
}
